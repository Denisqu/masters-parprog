    ...
    for (size = 2; size <= n; size *= 2) {
        ...
        for (i = 0; i < n; i += size) {
            for (j = i, k = 0; j < i + halfsize; j++, k += tablestep) {
                Vec2d temp_vec = [&]() -> Vec2d {
                    auto vec = Vec2d();
                    vec.load_a(pReal(inputVector[j + halfsize]));
                    vec *= expTable[k];
                    return vec;
                }();
                {
                    auto vec = Vec2d();
                    vec.load_a(pReal(inputVector[j]));
                    vec -= temp_vec;
                    vec.store_a(pReal(inputVector[j + halfsize]));
                }
                {
                    auto vec = Vec2d();
                    vec.load_a(pReal(inputVector[j]));
